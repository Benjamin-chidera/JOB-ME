...
export const RelatedJobs = ({ sliceRelatedJobs, status }: any) => {
        ...
        {status === "loading" ? (
          ...
          sliceRelatedJobs?.map(
            (job: {
              ...
              companyImage: string | StaticImport;
              companyName:
                | string
                | number
                | bigint
              ...
              <div
                key={job.id}
                className="w-[400px] border rounded-xl text-start"
              >
                <div className="p-5">
                  <h2 className=" font-semibold text-xl">{job.positions}</h2>

                  <p className=" text-gray-400 my-2 flex items-center gap-1">
                    <span>
                      <CiClock2 size={24} />
                    </span>{" "}
